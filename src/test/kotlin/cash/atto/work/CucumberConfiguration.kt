...
class CucumberConfiguration(
    ...
    companion object {
        val workGeneratedSubscription = "work-generated-subscription"
        val workRequestedTopic = "work-requested-topic"
        ...
        class PubSubInitializer(val environment: Environment, val pubSubProperties: PubSubProperties) {
            ...
            fun init() {
                ...
                admin.createTopic(workRequestedTopic)
                admin.createSubscription(pubSubProperties.workRequestedSubscription, workRequestedTopic)
                admin.createTopic(pubSubProperties.workGeneratedTopic)
                admin.createSubscription(workGeneratedSubscription, pubSubProperties.workGeneratedTopic)
                admin.close()
                channel.shutdown()
            }
        }
    ...
    fun teardown() {
        await().until(
            { subscriberTemplate.pullAndAck(pubSubProperties.workRequestedSubscription, Integer.MAX_VALUE, true) },
            hasSize(0)
        )

        await().until(
            { subscriberTemplate.pullAndAck(workGeneratedSubscription, Integer.MAX_VALUE, true) },
            hasSize(0)
        )
    }
